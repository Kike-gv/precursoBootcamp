var users = [
    {
      user: "admin",
      cardBoard: [0],
      cardBoardLines: [{ line1: [], line2: [], line3: [] }],
      cardBoardCompleted: ["O"],
      cardBoardCompletedLines: [{ line1: [], line2: [], line3: [] }],
      puntuacion: 999
    }
  ];
  
  var bingo = function bingo() { 
      /* si declaras una variable y la asignas a una funcion no hace falta ponerle el nombre a la funcion quedaria 
      var bingo = function()Â {...} lo mismo con el resto*/
      ...
      users.forEach(function(element) {
        if (element.user === user) {
          alert(
            "Cuanto tiempo " +
              user +
              ", encantado de volver a verte, tu record es de : " +
              element.puntuacion
          );
          oldUser = true;
        }
    ...
    var generateCardboard = function generateCardboard() {
      ...
      var changeCardboard = confirm(
        "tus numeros de bingo son : " + cardBoard + ", te gustan o los cambiamos?"
      );
      if (!changeCardboard) {
        generateCardboard();
      }
    ...
    var generateCardboardLines = function generateCardboardLines() {
      ...
      // No estamos usando numberOfLines deberiamos eliminar la linea sino la usamos
      var completedN = "O";
      users.forEach(function(element) {
        if (element.user === currentUser) {
          var aux = 0;
        ...
        if (element.user === currentUser) {
          ...
          if (foundN === false) {
            ...
            if (!bingo) {
              ...
              /* deberiamos solo llamar a checkLine una vez cuando haya la primera linea despues de que la primera linea haya 
              sido cantada no deberiamos informar al usuario en los proximos turnos que ha hecho linea */
              if (!totalLines) {
                console.log(element.cardBoardLines[0].line1);
                ...
                // Deberiamos aÃ±adir la condiciÃ³n de escape por si el usuario se cansa y no quiere seguir jugando
              }
            }
          }
        }
      ...
      users.forEach(function(element) {
        if (element.user === currentUser) {
          ...
          if (element.cardBoardCompleted.length === correctN) {
            ...
            var bingo = confirm(
              "Â¡Â¡Â¡BINGO!!! Has ganado, quieres intentar mejorar tu puntuacion?"
            );
            if (bingo) {
              isBingo = true;
            }
